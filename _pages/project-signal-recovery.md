...
where $\mathbf{\Pi}$ is a structured permutation matrix built from binary diagonal blocks $\mathbf{Q}_{mn} = \text{diag}(\mathbf{q}_{mn})$, with the constraint that each sample goes to exactly one channel: $\sum_m \mathbf{q}_{mn} = \sum_n \mathbf{q}_{mn} = \mathbf{1}$.
